...
@implementation MQTiledPDFView

// Create a new MQTiledPDFView with the desired frame and scale.
- (id)initWithFrame:(CGRect)frame andScale:(CGFloat)scale{
    if ((self = [super initWithFrame:frame])) {
		...
		// the view is done zooming a new MQTiledPDFView is created
...
@implementation MQPDFScrollView
...
{		
	...
	// Create a low res image representation of the PDF page to display before the MQTiledPDFView
	// renders its content.
	...
	// Create the MQTiledPDFView based on the size of the PDF page and scale it to fit the view.
	pdfView = [[MQTiledPDFView alloc] initWithFrame:pageRect andScale:pdfScale];
...
// We return our current MQTiledPDFView.
...
// we create a new MQTiledPDFView based on the new zoom level and draw it on top of the old MQTiledPDFView.
- (void)scrollViewDidEndZooming:(UIScrollView *)scrollView withView:(UIView *)view atScale:(float)scale
{
	CGRect pageRect = CGPDFPageGetBoxRect(page, kCGPDFMediaBox);

	// set the new scale factor for the MQTiledPDFView
	pdfScale *=scale;
#if 0
	...
	// Calculate the new frame for the new MQTiledPDFView
	if (YES)
	//if (is_interface_portrait())
		pageRect.size = CGSizeMake(pageRect.size.width*pdfScale, pageRect.size.height*pdfScale);
	else
	...
	// Create a new MQTiledPDFView based on new frame and scaling.
	pdfView = [[MQTiledPDFView alloc] initWithFrame:pageRect andScale:pdfScale];
	[pdfView setPage:page];
	
	// Add the new MQTiledPDFView to the MQPDFScrollView.
	[self addSubview:pdfView];
...
// we remove the old MQTiledPDFView and set the current MQTiledPDFView to be the old view so we can create a
// a new MQTiledPDFView when the zooming ends.
- (void)scrollViewWillBeginZooming:(UIScrollView *)scrollView withView:(UIView *)view
{
	...
	// Set the current MQTiledPDFView to be the old view.
	oldPDFView = pdfView;
...
@implementation MQPDFViewController
...
- (void)loadView {
	...
	// Create our MQPDFScrollView and add it to the view controller.
	//	scroll_pdf = [[MQPDFScrollView alloc] initWithFrame:[[self view] bounds]];
