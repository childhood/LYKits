...
static MQRandom *mq_shared_manager = nil;

@implementation MQRandom
...
+ (id)manager
{
	@synchronized(self)
	{
		if (mq_shared_manager == nil)
			[[self alloc] init];
	}
	return mq_shared_manager;
}
+ (id)allocWithZone:(NSZone *)zone
{
	@synchronized(self)
	{
		if (mq_shared_manager == nil)
		{
			mq_shared_manager = [super allocWithZone:zone];
			return mq_shared_manager;
		}
	}
	return nil;
...
{
	return [MQRandom contents_of_file:[@"mq_database_national_motto.txt" filename_bundle] separator:@"\n"];
}
