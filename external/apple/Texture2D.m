...
#define _deg_to_rad(__ANGLE)		((__ANGLE) / 180.0 * M_PI)
...
- (id) initWithImage:(UIImage *)uiImage
{
	...
	NSLog(@"image pixel format: %i", pixelFormat);
...
- (id) initWithString:(NSString*)string dimensions:(CGSize)dimensions alignment:(UITextAlignment)alignment fontName:(NSString*)name fontSize:(CGFloat)size
{
	...
	CGContextAddLineToPoint(context, width, height * 0.75);
	CGContextAddLineToPoint(context, width * 0.5, height);
	CGContextAddLineToPoint(context, 0, height * 0.75);
	CGContextAddLineToPoint(context, 0, 0);
	CGContextStrokePath(context);
	//CGContextStrokeRectWithWidth(context, CGRectMake(0, 0, width, height), 1.0);
#endif
	UIGraphicsPopContext();

	NSLog(@"text pixel format: %i", kTexture2DPixelFormat_A8);
...
double latitude_to_km(double latitude)
{
	return latitude * 110.57461087757687;
}

double longitude_to_km(double longitude, double latitude)
{
	return longitude * 2 * 3.1415926 * 6371 / 360 * cos(_deg_to_rad(latitude));
}
