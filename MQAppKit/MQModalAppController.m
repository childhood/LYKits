...
@implementation MQModalAppController
...
- (UIView*)get_view_named:(NSString*)name
{
	...
	UIView<MQModalAppSubview>*	subview;
	...
	for (i = 0; i < data.count; i++)
	{
		...
		item_name	= [dict objectForKey:k_mq_app_name];
		//	NSLog(@"item_name:%@",item_name);
		if ([item_name isEqualToString:name])
		{
			array_subviews = [dict objectForKey:k_mq_app_subview];
			for (ii = 0; ii < array_subviews.count; ii++)
			{
				dict_subview	= [array_subviews objectAtIndex:ii];
				subview_class	= [dict_subview objectForKey:k_mq_subview_class];
				subview_arg		= [dict_subview objectForKey:k_mq_subview_arg];
				subview_rect	= [dict_subview objectForKey:k_mq_subview_frame];
